...
export class SalaryStructureController {
  ...
  static getAll = asyncHandler(async (req: IAuthRequest, res: Response) => {
    ...
    const result = await SalaryStructureService.getAll(query, effectiveCompany(req.user));
    res.status(200).json(
  ...
  static getById = asyncHandler(async (req: IAuthRequest, res: Response) => {
    const doc = await SalaryStructureService.getById(req.params.id as string, effectiveCompany(req.user));
    res.status(200).json(buildResponse(true, doc, 'Salary Structure retrieved successfully'));
  });

  static create = asyncHandler(async (req: IAuthRequest, res: Response) => {
    const doc = await SalaryStructureService.create({ ...req.body, company: effectiveCompany(req.user) });
    res.status(201).json(buildResponse(true, doc, 'Salary Structure created successfully'));
  });

  static update = asyncHandler(async (req: IAuthRequest, res: Response) => {
    const doc = await SalaryStructureService.update(req.params.id as string, req.body, effectiveCompany(req.user));
    res.status(200).json(buildResponse(true, doc, 'Salary Structure updated successfully'));
  });

  static delete = asyncHandler(async (req: IAuthRequest, res: Response) => {
    const result = await SalaryStructureService.delete(req.params.id as string, effectiveCompany(req.user));
  ...
  static getAssignedHeads = asyncHandler(async (req: IAuthRequest, res: Response) => {
    const rows = await SalaryStructureService.getAssignedHeads(req.params.id as string, effectiveCompany(req.user));
  ...
  static assignHead = asyncHandler(async (req: IAuthRequest, res: Response) => {
    const row = await SalaryStructureService.assignHead({ ...req.body, company: effectiveCompany(req.user) });
  ...
  static updateAssignedHead = asyncHandler(async (req: IAuthRequest, res: Response) => {
    const row = await SalaryStructureService.updateAssignedHead(req.params.assignmentId as string, req.body, effectiveCompany(req.user));
  ...
  static removeAssignedHead = asyncHandler(async (req: IAuthRequest, res: Response) => {
    const result = await SalaryStructureService.removeAssignedHead(req.params.assignmentId as string, effectiveCompany(req.user));
  ...
  static bulkAssignToEmployees = asyncHandler(async (req: IAuthRequest, res: Response) => {
    ...
    const r = await SalaryStructureService.bulkAssignToEmployees(
      employeeIds, structure, { pfApplicable, esicApplicable }, effectiveCompany(req.user),
    );
  ...
  static bulkAppraisal = asyncHandler(async (req: IAuthRequest, res: Response) => {
    ...
    const r = await SalaryStructureService.bulkAppraisal(employeeIds, percent, effectiveCompany(req.user));
