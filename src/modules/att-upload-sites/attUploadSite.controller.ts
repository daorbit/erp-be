...
export class AttUploadSiteController {
  static getAll = asyncHandler(async (req: IAuthRequest, res: Response) => {
    ...
    const r = await AttUploadSiteService.getAll(q, effectiveCompany(req.user));
    res.status(200).json(buildResponse(true, r.data, 'Att Upload Sites retrieved', r.pagination));
  });
  static create = asyncHandler(async (req: IAuthRequest, res: Response) => {
    res.status(201).json(buildResponse(true, await AttUploadSiteService.create({ ...req.body, company: effectiveCompany(req.user) }), 'Created'));
  });
  static delete = asyncHandler(async (req: IAuthRequest, res: Response) => {
    res.status(200).json(buildResponse(true, await AttUploadSiteService.delete(req.params.id as string, effectiveCompany(req.user)), 'Deleted'));
  });
}
